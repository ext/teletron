...
class MCP:
    ...
    @staticmethod
    def gen(self, txt):
        return md5.new(str(self.disc.uid) + txt.upper()).hexdigest()[:8]

    @expose
    def shutdown(self, key):
        """TERMINATE MCP
NEEDS PERSONAL KEY GENERATED BY PWDGEN.
USAGE: SHUTDOWN <PERSONAL KEY>"""
        ok = [MCP.gen(self, 'IS THIS REAL LIFE'), MCP.gen(self, 'IS THIS REAL LIFE?')]
        if key not in ok:
            print >> self, 'INVALID PERSONAL KEY'
            return
    ...
    @expose
    def pwdgen(self, *key):
        """GENERATE PERSONAL SHUTDOWN KEY.
